...
function submission() {
	...
	.done((response)=> {
		...
		Object.keys(response).forEach((element)=> {
			...
			let result = document.createTextNode(element+" investment has a value of " + response[element].total.toFixed(2) + " with " + response[element].stock.toFixed(2) +" in stock and "+ response[element].leftover.toFixed(2) + " in cash for a total % return of: "+(100*(response[element].percent-1)).toFixed(2)+"\n");
